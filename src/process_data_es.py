...
import utils_es, utils
from process_data import LoadPromptData
from config import ES_ENCODING


class LoadPromptDataEs(LoadPromptData):
    def __init__(self, tokenizer, params):
        super().__init__(tokenizer, params)
        self.encoding = ES_ENCODING
    ...
    def build_prompt(self, context, token, context_window, prompt1, prompt2, template):
        return utils_es.build_prompt(context, token, context_window, prompt1, prompt2, template, self.tokenizer)
    ...
    def build_labels(self, context, token, gold_subs, context_window, prompt1, prompt2, template, top_k):
        ...
        prompts = utils_es.get_es_prompts(token, left_context, right_context, context_window, prompt1, prompt2,
